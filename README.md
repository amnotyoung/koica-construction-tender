...
> 데이터 비공개·부재로 해석하지 않는다. 유사사례는 공개 Supabase RPC를 먼저
> 조회하고, 전용 MCP 도구와 로컬 또는 GitHub의 SQLite를 보조 경로로 사용한다.
> 한 경로의 실패는 그 경로의
...
공사계약으로 오인하지 않는다. 종료평가 대조 175개 사업, 보고서 42개, 채택
매칭 46건과 공개 승인된 건축 근거 158건도 함께 제공한다. 사례 검색 결과에는
종료평가 대조 상태와 근거 수가 붙고, 사례 상세에서는 보고서 제목·공식 URL·물리
PDF 페이지·근거 발췌를 바로 확인할 수 있다. 실제 원격 배포 건수와 원본 DB
...
3. `get_koica_construction_case`: 선택 사례와 동일 사업의 관련 공고·종료평가 근거 확인

도구가 없는 환경에서도 데이터가 없는 것은 아니다. 권장 경로인 공개 Supabase
RPC를 호출하고, 전체 내부 색인이 필요할 때만 SQLite를 직접 조회한다.
...
- Supabase에는 공개 검색에 필요한 축약 사례·관련 공고와 공개 승인된 종료평가
  근거를 적재한다. 조달 원문 근거, 로컬 파일경로, 개별 파일·첨부 해시, OCR
  다이제스트와 내부 검토 메모는 제외하고, 동기화 원본을 식별하는 SQLite 스키마
  버전과 DB SHA-256만 상태 RPC에 공개한다.
